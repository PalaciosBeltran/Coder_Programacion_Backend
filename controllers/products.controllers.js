...
const getFakeProducts = async (req, res) => {
  let fakeProducts = await productsDao.populate();
  const response = successResponse(fakeProducts);
  res.status(HTTP_STATUS.OK).json(response);  
}

const productsController = { getProducts, getProductByID, saveNewProduct, updateCurrentProduct, deleteProduct, getFakeProducts};
